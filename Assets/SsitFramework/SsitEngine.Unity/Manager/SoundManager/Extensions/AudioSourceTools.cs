...
namespace SsitEngine.Unity.Sound
{
    public static class AudioSourceTools
    {
        ...
        {
            SoundManager.PlaySFX(theAudioSource,
                fromGroup ? SoundManager.LoadFromGroup(clipOrGroup_Name) : SoundManager.Load(clipOrGroup_Name), loop,
                delay, volume, pitch);
        }
        ...
        {
            SoundManager.PlaySFX(theAudioSource,
                fromGroup ? SoundManager.LoadFromGroup(clipOrGroup_Name) : SoundManager.Load(clipOrGroup_Name));
        }
        ...
        public static void PlaySFXLoop( ref AudioSource theAudioSource, bool fromGroup, string clipOrGroup_Name,
            bool tillDestroy, float volume, float pitch, float maxDuration )
        {
            SoundManager.PlaySFXLoop(theAudioSource,
                fromGroup ? SoundManager.LoadFromGroup(clipOrGroup_Name) : SoundManager.Load(clipOrGroup_Name),
                tillDestroy, volume, pitch, maxDuration);
        }

        public static void PlaySFXLoop( ref AudioSource theAudioSource, bool fromGroup, string clipOrGroup_Name,
            bool tillDestroy, float volume, float pitch )
        {
            SoundManager.PlaySFXLoop(theAudioSource,
                fromGroup ? SoundManager.LoadFromGroup(clipOrGroup_Name) : SoundManager.Load(clipOrGroup_Name),
                tillDestroy, volume, pitch);
        }

        public static void PlaySFXLoop( ref AudioSource theAudioSource, bool fromGroup, string clipOrGroup_Name,
            bool tillDestroy, float volume )
        {
            SoundManager.PlaySFXLoop(theAudioSource,
                fromGroup ? SoundManager.LoadFromGroup(clipOrGroup_Name) : SoundManager.Load(clipOrGroup_Name),
                tillDestroy, volume);
