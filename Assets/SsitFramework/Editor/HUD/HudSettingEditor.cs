...
namespace SsitEngine.Editor.HUD
{
    ...
    public class HudSettingEditor : HudElementSettingEditorBase
    {
        ...
        protected override void OnChildInspectorGUI()
        {
            ...
            settingsReference = (GameObject) EditorGUILayout.ObjectField(
                new GUIContent("Copy From", "Assign the GameObject from which you want to extract the settings."),
                settingsReference, typeof(GameObject), true);
            if (settingsReference != null)
            {
                GUILayout.Space(4); // SPACE
                var element = settingsReference.GetComponent<HudElement>();
                if (element != null)
                {
                    // show paste button
                ...
                {
                    // show error message
                    EditorGUILayout.HelpBox("No HUDNavigationElement component found on GameObject.",
                        MessageType.Error);
                }
            }
            GUILayout.Space(4); // SPACE
