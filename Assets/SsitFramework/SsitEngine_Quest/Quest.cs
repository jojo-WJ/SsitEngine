...
namespace SsitEngine.QuestManager
{
    ...
    public class Quest : ScriptableObject
    {
        ...
        [Tooltip("提供此任务的任务给予者的ID。通常在发问者接受任务时在发问者的运行时实例上设置")]
        [SerializeField]
        ...
        [Tooltip("任务完成模式")]
        [SerializeField]
        private QuestCompleteMode m_completeMode;

        [Tooltip("如果被放弃，是否保存在任务日志中.")]
        [SerializeField]
        private bool m_rememberIfAbandoned;

        [Tooltip("如果指定，则条件满足时自动启动任务")]
        [SerializeField]
        private QuestConditionSet m_autostartConditionSet;

        [Tooltip("在提供任务之前必须满足的条件")]
        [SerializeField]
        private QuestConditionSet m_offerConditionSet;

        [Tooltip("提供条件不满足时显示的对话")]
        [SerializeField]
        ...
        [HideInInspector]
        [SerializeField]
        private string m_goalEntityTypeName = null;//目标实体类型名称
        ...
        public bool ShowInTrackHud
        {
            ...
            set
            {
                ...
                QuestUtility.SendNotification((ushort)EnQuestEvent.QuestTrackToggleChangedMessage, this, null, Id, value);
            }
        }
        ...
        private void OnDestroy()
        {
            if (IsInstance && Application.isPlaying)
            {
                ...
                if (this.GetState() != QuestState.Disabled)
        ...
        public void AssignQuester(QuestParticipantTextInfo questerTextInfo, QuestCompleteMode completeMode = QuestCompleteMode.SingleComplet)
        {
            if (questerTextInfo == null || string.IsNullOrEmpty(questerTextInfo.Id))
                return;
            TagDictionary.SetTag(QuestMachineTags.QUESTERID, questerTextInfo.Id);
            TagDictionary.SetTag(QuestMachineTags.QUESTER, questerTextInfo.DisplayName);
            this.m_completeMode = completeMode;
        }
        ...
        public QuestCounter GetCounter(string counterName)
        {
            ...
            {
                var counter = CounterList[i];
                if (counter != null && string.Equals(counter.name, counterName)) return counter;
            }
            return null;
        }
        ...
        public void SetQuestIndicatorState(string entityID, QuestIndicatorState questIndicatorState)
        {
            ...
            QuestUtility.SendNotification((ushort)EnQuestEvent.SetIndicatorStateMessage, this, entityID, Id, questIndicatorState);
        }
        ...
        public void ClearQuestIndicatorStates()
        {
            ...
            foreach (var kvp in IndicatorStates)
            {
                ...
                QuestUtility.SendNotification((ushort)EnQuestEvent.SetIndicatorStateMessage, this, kvp.Key, Id, QuestIndicatorState.None);
            }
