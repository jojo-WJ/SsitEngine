...
{
    [vClassHeader("Ragdoll System", true, "ragdollIcon", true,
        "Every gameobject children of the character must have their tag added in the IgnoreTag List.")]
    public class vRagdoll : vMonoBehaviour
    {
        private void Start()
        {
            ...
            if (iChar) iChar.onActiveRagdoll.AddListener(ActivateRagdoll);
            ...
            var components = GetComponentsInChildren(typeof(Transform));
        ...
        {
            var rdStabilize = Mathf.Infinity;
        ...
        private void RagdollBehaviour()
        {
            ...
            if (state == RagdollState.blendToAnim)
            {
                if (Time.time <= ragdollingEndTime + mecanimToGetUpTransitionTime)
                {
                    ...
                    var animatedToRagdolled =
                        ragdolledHipPosition - animator.GetBoneTransform(HumanBodyBones.Hips).position;
                    var newRootPosition = transform.position + animatedToRagdolled;

                    //Now cast a ray from the computed position downwards and find the highest hit that does not belong to the character 
                    var hits = Physics.RaycastAll(new Ray(newRootPosition + Vector3.up, Vector3.down));
                    //newRootPosition.y = 0;
                    ...
                    var ragdolledDirection = ragdolledHeadPosition - ragdolledFeetPosition;
                    ragdolledDirection.y = 0;

                    var meanFeetPosition = 0.5f * (animator.GetBoneTransform(HumanBodyBones.LeftFoot).position +
                                                   animator.GetBoneTransform(HumanBodyBones.RightFoot).position);
                    var animatedDirection = animator.GetBoneTransform(HumanBodyBones.Head).position - meanFeetPosition;
                ...
                var ragdollBlendAmount = 1.0f - (Time.time - ragdollingEndTime - mecanimToGetUpTransitionTime) /
                    ...
                    if (b.transform != transform)
                    {
                        ...
                        if (b.transform == animator.GetBoneTransform(HumanBodyBones.Hips))
                            b.transform.position =
                                Vector3.Lerp(b.transform.position, b.storedPosition, ragdollBlendAmount);
                        //rotation is interpolated for all body parts
                        b.transform.rotation =
                            Quaternion.Slerp(b.transform.rotation, b.storedRotation, ragdollBlendAmount);
                    }
        ...
        private void setKinematic( bool newValue )
        {
            ...
            var components = _hips.transform.GetComponentsInChildren(typeof(Rigidbody));
        ...
        private void setCollider( bool newValue )
        {
            ...
            var components = _hips.transform.GetComponentsInChildren(typeof(Collider));

            foreach (var c in components)
                if (!ignoreTags.Contains(c.transform.tag))
                    if (!c.transform.Equals(transform))
        ...
        [Tooltip(
            "SHOOTER: Keep false to use detection hit on each children collider, don't forget to change the layer to BodyPart from hips to all childrens. MELEE: Keep true to only hit the main Capsule Collider.")]
        ...
        [HideInInspector] public vCharacter iChar;

        private Animator animator;
        [HideInInspector] public Transform characterChest, characterHips;

        private bool inStabilize, isActive, updateBehaviour;
            ...
            set
            {
                ...
                else
                {
                    ...
                    if (state == RagdollState.ragdolled)
                    {
                        ...
                        ragdolledFeetPosition = 0.5f * (animator.GetBoneTransform(HumanBodyBones.LeftToes).position +
