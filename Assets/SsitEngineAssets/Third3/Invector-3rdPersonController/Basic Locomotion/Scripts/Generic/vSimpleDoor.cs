...
public class vSimpleDoor : vMonoBehaviour
{
    ...
    private void OnTriggerStay( Collider collider )
    {
        ...
        {
            forwardDotVelocity =
                Mathf.Abs(Vector3.Angle(transform.forward, collider.transform.position - transform.position));
            ...
            if (canOpen && !isOpen) StartCoroutine(_Open());
        }
    ...
    {
        if (autoClose && isOpen && tagsToOpen.Contains(collider.tag)) StartCoroutine(_Close());
    }
}