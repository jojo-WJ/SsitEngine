...
namespace Invector
{
    public static class vExtensions
    {
        ...
        {
            if (arrayToAppend == null) throw new ArgumentNullException("The appended object cannot be null");
        ...
        public static List<T> vCopy<T>( this List<T> list )
        {
            ...
            for (var i = 0; i < list.Count; i++) _list.Add(list[i]);
            return _list;
        }
        ...
        {
            var bp = new HitBarPoints();
            var boxPoint = boxCollider.GetBoxPoint();
            var toTop = new Ray(boxPoint.top, boxPoint.top - torso.position);
            var toCenter = new Ray(torso.position, boxPoint.center - torso.position);
            var toBottom = new Ray(torso.position, boxPoint.bottom - torso.position);
