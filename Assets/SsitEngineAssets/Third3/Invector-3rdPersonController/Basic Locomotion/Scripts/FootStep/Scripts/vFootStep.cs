...
public class vFootStep : vFootPlantingPlayer
{
    ...
    private void Start()
    {
        ...
        else
        {
            ...
            {
                var coll = colls[i];
                for (var a = 0; a < footStepTriggers.Count; a++)
                {
    ...
    public void StepOnTerrain( FootStepObject footStepObject )
    {
        ...
        //var str = (terrainData != null && terrainData.splatPrototypes.Length > 0)? ( terrainData.splatPrototypes[surfaceIndex]).texture.name:"" ;
        var str = terrainData != null && terrainData.terrainLayers.Length > 0
            ? terrainData.terrainLayers[surfaceIndex].diffuseTexture.name
            : "";
        footStepObject.name = str;
