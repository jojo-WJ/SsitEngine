...
{
    private Vector3 headPoint => transform.position + transform.up * headHeight;

    private bool lookConditions => head != null && followCamera && Camera.main != null ||
                                   !followCamera && (lookTarget || simpleTarget) || temporaryLookTime > 0;
    ...
    private void Start()
    {
        ...
        if (neck.parent != spine2) spines.Add(neck.parent);
        ...
        for (var i = 0; i < animatorTags.Count; i++) tagsHash.Add(Animator.StringToHash(animatorTags[i]));
        GetLookPoint();
    }
    ...
    public virtual void SetLookAtPosition( Vector3 point, float strafeHeadWeight, float spineWeight )
    {
        ...
        foreach (var segment in spines)
        {
            var rotX = Quaternion.AngleAxis(xAngle * spineWeight / spines.Count,
                segment.InverseTransformDirection(transform.right));
            var rotY = Quaternion.AngleAxis(yAngle * spineWeight / spines.Count,
    ...
    private Vector3 GetLookPoint()
    {
        ...
        if (lookConditions && !IgnoreHeadTrack())
        {
            ...
            if (temporaryLookTime <= 0)
            {
                ...
                {
                    if (lookTarget != null && TargetIsOnRange(lookTarget.lookPoint - headPoint) &&
                        lookTarget.IsVisible(headPoint, obstacleLayer))
                        dir = lookTarget.lookPoint - headPoint;
                    else if (simpleTarget != null) dir = simpleTarget.position - headPoint;
                }
            }
            else
            {
    ...
    private Vector2 GetTargetAngle( Vector3 direction )
    {
        ...
        var desiredRotation = Quaternion.Euler(angleResult); // convert angleResult to Rotation
        var x = (float) Math.Round(NormalizeAngle(desiredRotation.eulerAngles.x), 2);
        var y = (float) Math.Round(NormalizeAngle(desiredRotation.eulerAngles.y), 2);
    ...
    private void SmoothValues( float _headWeight = 0, float _bodyWeight = 0, float _x = 0, float _y = 0 )
    {
    ...
    private void SortTargets()
    {
        ...
        if (interation > updateTargetInteration)
        {
            ...
            for (var i = targetsInArea.Count - 1; i >= 0; i--)
                if (targetsInArea[i] == null)
                    targetsInArea.RemoveAt(i);
            targetsInArea.Sort(delegate( vLookTarget c1, vLookTarget c2 )
            {
                return Vector3
                    .Distance(transform.position, c1 != null ? c1.transform.position : Vector3.one * Mathf.Infinity)
                    .CompareTo
                    (Vector3.Distance(transform.position,
                        c2 != null ? c2.transform.position : Vector3.one * Mathf.Infinity));
            });
            if (targetsInArea.Count > 0) lookTarget = targetsInArea[0];
        }
    }
    ...
    public void OnLost( Collider other )
    {
        if (tagsToDetect.Contains(other.gameObject.tag) && other.GetComponentInParent<vLookTarget>() != null)
        {
            ...
            if (targetsInArea.Contains(lookTarget)) targetsInArea.Remove(lookTarget);
            SortTargets();
    ...
    public bool IgnoreHeadTrack()
    {
        for (var index = 0; index < animator.layerCount; index++)
        {
            ...
            if (tagsHash.Contains(info.tagHash)) return true;
        }
        return false;
    }
    ...
    [HideInInspector] public float minAngleX = -90f, maxAngleX = 90, minAngleY = -90f, maxAngleY = 90f;
    ...
    [vHideInInspector("followCamera")] public bool awaysFollowCamera;
    ...
    [HideInInspector] public bool updateIK;
    ...
    private Vector2 cameraAngle, targetAngle;

    [HideInInspector] public List<Transform> spines;

    private float yAngle, xAngle;
    private float _yAngle, _xAngle;

    [HideInInspector] public UnityEvent onInitUpdate = new UnityEvent();

    [HideInInspector] public UnityEvent onFinishUpdate = new UnityEvent();
