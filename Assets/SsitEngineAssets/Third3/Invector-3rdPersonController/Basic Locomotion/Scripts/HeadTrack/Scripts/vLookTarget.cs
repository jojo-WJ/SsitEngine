...
public class vLookTarget : MonoBehaviour
{
    ...
    private void DrawBox()
    {
        ...
        if (visibleCheckType == VisibleCheckType.BoxCast)
        {
            ...
            var rotationMatrix = Matrix4x4.TRS(transform.position + new Vector3(centerX, centerY, centerZ),
                transform.rotation, new Vector3(sizeX, sizeY, sizeZ) * 2f);
...
public static class vLookTargetHelper
{
    ...
    {
        var points = new LookPoints();
        var centerArea = lookTarget.centerArea;
        var sizeArea = lookTarget.sizeArea;
        var lookTransform = lookTarget.transform;
        points.frontTopLeft =
            new Vector3(centerArea.x - sizeArea.x, centerArea.y + sizeArea.y, centerArea.z - sizeArea.z);
    ...
    public static bool IsVisible( this vLookTarget lookTarget, Vector3 from, LayerMask layerMask, bool debug = false )
    {
        ...
        {
            if (lookTarget.useLimitToDetect &&
                Vector3.Distance(from, lookTarget.transform.position) > lookTarget.minDistanceToDetect) return false;
        ...
        {
            if (lookTarget.useLimitToDetect &&
                Vector3.Distance(from, lookTarget.centerArea) > lookTarget.minDistanceToDetect) return false;
            if (CastPoint(from, lookTarget.transform.TransformPoint(lookTarget.centerArea), lookTarget.transform,
                layerMask, debug)) return true;
            return false;
        }
        if (lookTarget.visibleCheckType == vLookTarget.VisibleCheckType.BoxCast)
        {
            if (lookTarget.useLimitToDetect &&
                Vector3.Distance(from, lookTarget.transform.position) > lookTarget.minDistanceToDetect) return false;
            var points = GetLookPoints(lookTarget);
    ...
    public static bool IsVisible( this vLookTarget lookTarget, Vector3 from, bool debug = false )
    {
        if (lookTarget.HideObject) return false;
        var points = GetLookPoints(lookTarget);
        if (lookTarget.visibleCheckType == vLookTarget.VisibleCheckType.None) return true;
        if (lookTarget.visibleCheckType == vLookTarget.VisibleCheckType.SingleCast)
        {
            if (CastPoint(from, lookTarget.transform.TransformPoint(lookTarget.centerArea), lookTarget.transform,
                debug)) return true;
    ...
    private static bool CastPoint( Vector3 from, Vector3 point, Transform lookTarget, LayerMask layerMask,
        bool debug = false )
    {
        RaycastHit hit;
