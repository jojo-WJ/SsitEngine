...
namespace RootMotion.FinalIK
{
    ...
    public class FullBodyBipedIK : IK
    {
        ...
        public bool ReferencesWarning( ref string warningMessage )
        {
            ...
            var toRightShoulder = references.rightUpperArm.position - references.leftUpperArm.position;
            var shoulderToRootNode = solver.rootNode.position - references.leftUpperArm.position;
            var dot = Vector3.Dot(toRightShoulder.normalized, shoulderToRootNode.normalized);

            if (dot > 0.95f)
            {
                warningMessage =
                    "The root node, the left upper arm and the right upper arm bones should ideally form a triangle that is as close to equilateral as possible. " +
            ...
            var toRightThigh = references.rightThigh.position - references.leftThigh.position;
            var thighToRootNode = solver.rootNode.position - references.leftThigh.position;
            dot = Vector3.Dot(toRightThigh.normalized, thighToRootNode.normalized);

            if (dot > 0.95f)
            {
                warningMessage =
                    "The root node, the left thigh and the right thigh bones should ideally form a triangle that is as close to equilateral as possible. " +
