...
namespace RootMotion.FinalIK
{
    ...
    public partial class Grounding
    {
        ...
        public float GetVerticalOffset( Vector3 p1, Vector3 p2 )
        {
            if (useRootRotation)
            {
                var v = Quaternion.Inverse(root.rotation) * (p1 - p2);
                return v.y;
            }
        ...
        public float footCenterOffset; // TODO make visible in inspector if Grounder Visualization is finished.
        ...
        public RaycastHit GetRootHit( float maxDistanceMlp = 10f )
        {
            ...
            var legsCenter = Vector3.zero;
            foreach (var leg in legs) legsCenter += leg.transform.position;
            legsCenter /= (float) legs.Length;

            h.point = legsCenter - _up * maxStep * 10f;
            var distMlp = maxDistanceMlp + 1;
            h.distance = maxStep * distMlp;

            if (maxStep <= 0f) return h;

            if (quality != Quality.Best)
                Physics.Raycast(legsCenter + _up * maxStep, -_up, out h, maxStep * distMlp, layers);
            else
                Physics.SphereCast(legsCenter + _up * maxStep, rootSphereCastRadius, -up, out h, maxStep * distMlp,
                    layers);
        ...
        public void Initiate( Transform root, Transform[] feet )
        {
            ...
            {
                for (var i = 0; i < feet.Length; i++) legs[i].Initiate(this, feet[i]);
        ...
        public void Update()
        {
            ...
            var lowestOffset = Mathf.NegativeInfinity;
            var highestOffset = Mathf.Infinity;
            isGrounded = false;
        ...
        public Vector3 GetLegsPlaneNormal()
        {
            ...
            for (var i = 0; i < legs.Length; i++)
            {
                ...
                var legDirection = legs[i].IKPosition - root.position;

                // Find the tangent
                var legNormal = _up;
                var legTangent = legDirection;
                Vector3.OrthoNormalize(ref legNormal, ref legTangent);

                // Find the rotation offset from the tangent to the direction
                var fromTo = Quaternion.FromToRotation(legTangent, legDirection);
        ...
        public void Reset()
        {
            ...
            pelvis.Reset();
            foreach (var leg in legs) leg.Reset();
        }
