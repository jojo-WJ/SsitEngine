    ...
    public class GrounderIK : Grounder
    {
        ...
        private void Update()
        {
            ...
            if (initiated)
            {
                ...
                {
                    var normal = solver.GetLegsPlaneNormal();

                    // Root rotation weight
                    if (rootRotationWeight < 1f) normal = Vector3.Slerp(Vector3.up, normal, rootRotationWeight);

                    // Root rotation limit
                    var upRotation = Quaternion.FromToRotation(transform.up, Vector3.up) * characterRoot.rotation;
                    var rotationTarget = Quaternion.RotateTowards(upRotation,
                        Quaternion.FromToRotation(transform.up, normal) * characterRoot.rotation, maxRootRotationAngle);

                    // Rotate the root
                    characterRoot.rotation = Quaternion.Lerp(characterRoot.rotation, rotationTarget,
        ...
        private void Initiate()
        {
            ...
            for (var i = 0; i < feet.Length; i++) footRotations[i] = Quaternion.identity;

            // Gathering the last bones of the IK solvers as feet
            for (var i = 0; i < legs.Length; i++)
            {
                var points = legs[i].GetIKSolver().GetPoints();
        ...
        private void OnSolverUpdate()
        {
            ...
            for (var i = 0; i < legs.Length; i++) SetLegIK(i);
        ...
        private void SetLegIK( int index )
        {
            ...
            if (legs[index] is LegIK)
            {
                (legs[index].GetIKSolver() as IKSolverLeg).IKRotation =
                    Quaternion.Slerp(Quaternion.identity, solver.legs[index].rotationOffset, weight) *
                    footRotations[index];
