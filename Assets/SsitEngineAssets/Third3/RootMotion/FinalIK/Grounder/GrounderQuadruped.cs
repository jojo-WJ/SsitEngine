...
namespace RootMotion.FinalIK
{
    ...
    public class GrounderQuadruped : Grounder
    {
        ...
        {
            foreach (var leg in ikComponents)
            {
        ...
        private void Initiate()
        {
            ...
            var footBones = InitiateFeet(legs, ref feet, 0);
            var forefootBones = InitiateFeet(forelegs, ref feet, legs.Length);
            ...
            for (var i = 0; i < footBones.Length; i++) feet[i].leg = solver.legs[i];
            for (var i = 0; i < forefootBones.Length; i++) feet[i + legs.Length].leg = forelegSolver.legs[i];
        ...
        {
            var bones = new Transform[ikComponents.Length];

            for (var i = 0; i < ikComponents.Length; i++)
            {
                var points = ikComponents[i].GetIKSolver().GetPoints();
        ...
        private void RootRotation()
        {
            ...
            var tangent = characterRoot.forward;

            var normal = -gravity;
            Vector3.OrthoNormalize(ref normal, ref tangent);
            var horizontalRotation = Quaternion.LookRotation(tangent, -gravity);

            // Get the direction from root hit to forelegs root hit in the space of the horizontal character rotation
            var hitDirection = forelegSolver.rootHit.point - solver.rootHit.point;
            var hitDirectionLocal = Quaternion.Inverse(horizontalRotation) * hitDirection;

            // Get the angle between the horizontal and hit directions
            var angleTarget = Mathf.Atan2(hitDirectionLocal.y, hitDirectionLocal.z) * Mathf.Rad2Deg;
            ...
            characterRoot.rotation = Quaternion.Slerp(characterRoot.rotation,
                Quaternion.AngleAxis(-angle, characterRoot.right) * horizontalRotation, weight);
        }
        ...
        private void OnSolverUpdate()
        {
            ...
            for (var i = 0; i < feet.Length; i++) feet[i].rotation = feet[i].transform.rotation;
            ...
            // Rotate the pelvis
            var spineDirection = lastSpineBone.position - pelvis.position;

            var newSpinePosition =
            ...
            var newDirection = newSpinePosition - pelvis.position;

            var f = Quaternion.FromToRotation(spineDirection, newDirection);
            ...
            for (var i = 0; i < feet.Length; i++) SetFootIK(feet[i], i < 2 ? maxLegOffset : maxForeLegOffset);
        ...
        {
            // Get the centroid
            var foreFeetCenter = Vector3.zero;

            for (var i = 0; i < forelegSolver.legs.Length; i++)
                foreFeetCenter += forelegSolver.legs[i].transform.position;

            foreFeetCenter /= (float) forelegs.Length;
            var dir = foreFeetCenter - transform.position;
        ...
        {
            var direction = foot.leg.IKPosition - foot.transform.position;
        ...
        private void OnPostSolverUpdate()
        {
            ...
            for (var i = 0; i < feet.Length; i++)
                feet[i].transform.rotation = Quaternion.Slerp(Quaternion.identity, feet[i].leg.rotationOffset, weight) *
                                             feet[i].rotation;
