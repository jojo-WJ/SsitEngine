...
namespace RootMotion.FinalIK
{
    ...
    public class AimController : MonoBehaviour
    {
        ...
        private void LateUpdate()
        {
            ...
            // Smooth weight
            ik.solver.IKPositionWeight = Mathf.SmoothDamp(ik.solver.IKPositionWeight, target != null ? weight : 0f,
            ...
            {
                var targetDir = ik.solver.IKPosition - pivot;
            ...
            if (useAnimatedAimDirection)
                ik.solver.axis =
                    ik.solver.transform.InverseTransformVector(ik.transform.rotation * animatedAimDirection);
        }
        ...
        {
            var aimFrom = pivot;
            var direction = ik.solver.IKPosition - aimFrom;
        ...
        {
            var max = Mathf.Lerp(180f, maxRootAngle, ik.solver.IKPositionWeight);

            if (max < 180f)
            {
                var faceDirLocal = Quaternion.Inverse(ik.transform.rotation) * (ik.solver.IKPosition - pivot);
                var angle = Mathf.Atan2(faceDirLocal.x, faceDirLocal.z) * Mathf.Rad2Deg;

                var rotation = 0f;

                if (angle > max) rotation = angle - max;
                if (angle < -max) rotation = angle + max;
