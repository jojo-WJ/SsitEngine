...
namespace RootMotion.FinalIK
{
    ...
    public class ShoulderRotator : MonoBehaviour
    {
        ...
        private void RotateShoulder( FullBodyBipedChain chain, float weight, float offset )
        {
            ...
            var fromTo = Quaternion.FromToRotation(GetParentBoneMap(chain).swingDirection,
                ik.solver.GetEndEffector(chain).position - GetParentBoneMap(chain).transform.position);

            // Direction to the IK target
            var toTarget = ik.solver.GetEndEffector(chain).position - ik.solver.GetLimbMapping(chain).bone1.position;

            // Length of the limb
            var limbLength = ik.solver.GetChain(chain).nodes[0].length + ik.solver.GetChain(chain).nodes[1].length;

            // Divide IK Target direction magnitude by limb length to know how much the limb is being pulled
            var delta = toTarget.magnitude / limbLength - 1f + offset;
            delta = Mathf.Clamp(delta * weight, 0f, 1f);

            // Calculate the rotation offset for the shoulder
            var rotationOffset = Quaternion.Lerp(Quaternion.identity, fromTo,
                delta * ik.solver.GetEndEffector(chain).positionWeight * ik.solver.IKPositionWeight);

            // Rotate the shoulder
            ik.solver.GetLimbMapping(chain).parentBone.rotation =
                rotationOffset * ik.solver.GetLimbMapping(chain).parentBone.rotation;
        }
