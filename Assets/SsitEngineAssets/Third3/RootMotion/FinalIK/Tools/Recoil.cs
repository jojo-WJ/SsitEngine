...
namespace RootMotion.FinalIK
{
    ...
    public class Recoil : OffsetModifier
    {
        ...
        [Tooltip(
            "Set this true if you are using IKExecutionOrder.cs or a custom script to force AimIK solve after FBBIK.")]
        ...
        [Space(10)] [Tooltip("FBBIK effector position offsets for the recoil (in aiming direction space).")]
        ...
        {
            if (handedness == Handedness.Left) primaryHandRotation = leftHandRotation;
            //secondaryHandRotation = rightHandRotation;
            else primaryHandRotation = rightHandRotation;
        ...
        {
            var rnd = magnitude * Random.value * magnitudeRandom;
            magnitudeMlp = magnitude + rnd;

            randomRotation = Quaternion.Euler(rotationRandom * Random.value);

            foreach (var offset in offsets) offset.Start();
        ...
        protected override void OnModifyOffset()
        {
            ...
            var wTarget = recoilWeight.Evaluate(length - (endTime - Time.time)) * magnitudeMlp;
            w = Mathf.Lerp(w, wTarget, blendWeight);

            // Find the rotation space of the recoil
            var lookRotation = aimIK != null && aimIK.solver.transform != null && !aimIKSolvedLast
            ...
            foreach (var offset in offsets) offset.Apply(ik.solver, lookRotation, w, length, endTime - Time.time);

            if (!handRotationsSet) primaryHandRotation = primaryHand.rotation;
            ...
            if (twoHanded)
            {
                var secondaryHandRelativePosition = Quaternion.Inverse(primaryHand.rotation) *
                ...
                var primaryHandPosition = primaryHand.position + primaryHandEffector.positionOffset;
                var secondaryHandPosition = primaryHandPosition + handRotation * secondaryHandRelativePosition;

                secondaryHandEffector.positionOffset +=
                    secondaryHandPosition - (secondaryHand.position + secondaryHandEffector.positionOffset);
            }

            if (aimIK != null && aimIKSolvedLast)
                aimIK.solver.axis = Quaternion.Inverse(ik.references.root.rotation) *
                                    Quaternion.Inverse(rotationOffset) * aimIKAxis;
        }
        ...
        public class RecoilOffset
        {
                ...
                float timeLeft )
            {
                ...
                foreach (var e in effectorLinks) solver.GetEffector(e.effector).positionOffset += lastOffset * e.weight;
            }
