...
namespace RootMotion.FinalIK
{
    ...
    public class HitReactionVRIK : OffsetModifierVRIK
    {
        ...
        {
            foreach (var p in positionOffsets) p.Apply(ik, offsetCurves, weight);
            foreach (var r in rotationOffsets) r.Apply(ik, offsetCurves, weight);
        }
        ...
        public void Hit( Collider collider, Vector3 force, Vector3 point )
        {
            ...
            foreach (var p in positionOffsets)
        ...
        public class PositionOffset : Offset
        {
            ...
            {
                var time1 = curves[forceDirCurveIndex].keys.Length > 0
                    ? curves[forceDirCurveIndex].keys[curves[forceDirCurveIndex].length - 1].time
                    : 0f;
                var time2 = curves[upDirCurveIndex].keys.Length > 0
            ...
            {
                foreach (var l in offsetLinks) l.CrossFadeStart();
            }

            // Calculate offset, apply to FBBIK effectors
            protected override void OnApply( VRIK ik, AnimationCurve[] curves, float weight )
            {
                var up = ik.transform.up * force.magnitude;

                var offset = curves[forceDirCurveIndex].Evaluate(timer) * force +
                             curves[upDirCurveIndex].Evaluate(timer) * up;
                offset *= weight;

                foreach (var l in offsetLinks) l.Apply(ik, offset, crossFader);
            }
        ...
        public class RotationOffset : Offset
        {
            ...
            protected override void OnApply( VRIK ik, AnimationCurve[] curves, float weight )
            {
                ...
                if (rigidbody != null)
                {
                    ...
                    var comValue = curves[curveIndex].Evaluate(timer) * weight;
