...
namespace RootMotion.FinalIK
{
    ...
    public class InteractionObject : MonoBehaviour
    {
        ...
        [field: SerializeField] public bool NeedIkPick { get; set; } = true;
        ...
        {
            for (var i = 0; i < weightCurves.Length; i++)
            {
                var mlp = target == null ? 1f : target.GetValue(weightCurves[i].type);
            ...
            for (var i = 0; i < multipliers.Length; i++)
            {
                if (multipliers[i].curve == multipliers[i].result)
                    if (!Warning.logged)
                        Warning.Log(
                            "InteractionObject Multiplier 'Curve' " + multipliers[i].curve +
                            "and 'Result' are the same.", transform);

                var curveIndex = GetWeightCurveIndex(multipliers[i].curve);

                if (curveIndex != -1)
                {
                    var mlp = target == null ? 1f : target.GetValue(multipliers[i].result);

                    Apply(solver, effector, multipliers[i].result,
                        multipliers[i].GetValue(weightCurves[curveIndex], timer), weight * mlp);
                }
                else
                {
                    if (!Warning.logged)
                        Warning.Log("InteractionObject Multiplier curve " + multipliers[i].curve + "does not exist.",
                            transform);
                }
            }
        }
        ...
        {
            var index = GetWeightCurveIndex(weightCurveType);

            if (index != -1)
            {
                var mlp = target == null ? 1f : target.GetValue(weightCurveType);
            ...
            for (var i = 0; i < multipliers.Length; i++)
                ...
                {
                    var wIndex = GetWeightCurveIndex(multipliers[i].curve);
                    if (wIndex != -1)
                    {
                        var mlp = target == null ? 1f : target.GetValue(multipliers[i].result);
        ...
        public class InteractionEvent
        {
            ...
            {
                unityEvent.Invoke();
                foreach (var e in animations) e.Activate(pickUp);
                foreach (var m in messages) m.Send(t);
            }
        }
        ...
        public void Initiate()
        {
            ...
            for (var i = 0; i < weightCurves.Length; i++)
                ...
                {
                    var l = weightCurves[i].curve.keys[weightCurves[i].curve.length - 1].time;
                    length = Mathf.Clamp(length, l, length);
                }

            // Push length to the last event time
            for (var i = 0; i < events.Length; i++) length = Mathf.Clamp(length, events[i].time, length);
