    ...
    public class InteractionTrigger : MonoBehaviour
    {
        ...
        public class CharacterPosition
        {
            ...
            public bool IsInRange( Transform character, Transform trigger, out float error )
            {
                ...
                var forward = trigger.forward;
                if (fixYAxis) forward.y = 0f;
                if (forward == Vector3.zero) return false; // Singularity

                var up = fixYAxis ? Vector3.up : trigger.up;

                var triggerRotation = Quaternion.LookRotation(forward, up);

                var position = trigger.position + triggerRotation * offset3D;

                var origin = orbit ? trigger.position : position;
                var toCharacter = character.position - origin;
                ...
                if (orbit)
                {
                    ...
                    var dist = toCharacter.magnitude;
                ...
                var d = triggerRotation * direction3D;
                Vector3.OrthoNormalize(ref up, ref d);

                if (orbit)
                {
                    var toPosition = position - trigger.position;
                    if (toPosition == Vector3.zero) toPosition = Vector3.forward;
                    var r = Quaternion.LookRotation(toPosition, up);
                    toCharacter = Quaternion.Inverse(r) * toCharacter;

                    var a = Mathf.Atan2(toCharacter.x, toCharacter.z) * Mathf.Rad2Deg;
                    d = Quaternion.AngleAxis(a, up) * d;
                }

                var angle = Vector3.Angle(d, character.forward);
        ...
        public class CameraPosition
        {
            ...
            public Quaternion GetRotation()
            {
                ...
                var up = fixYAxis ? Vector3.up : lookAtTarget.transform.up;
            ...
            public bool IsInRange( Transform raycastFrom, RaycastHit hit, Transform trigger, out float error )
            {
                ...
                var d = GetRotation() * direction;

                var a = Vector3.Angle(raycastFrom.position - hit.point, d);
        ...
        public class Range
        {
            ...
            public string name; // Name is composed automatically by InteractionTriggerInspector.cs. Editor only.

            [HideInInspector] [SerializeField]
            public bool show = true; // Show this range in the Scene view? Editor only.
