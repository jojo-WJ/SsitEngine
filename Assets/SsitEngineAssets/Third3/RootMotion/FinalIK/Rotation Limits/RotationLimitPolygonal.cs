...
namespace RootMotion.FinalIK
{
    ...
    public class RotationLimitPolygonal : RotationLimit
    {
        ...
        private void Start()
        {
            ...
            for (var i = 0; i < reachCones.Length; i++)
                if (!reachCones[i].isValid)
                {
                    if (smoothIterations <= 0)
                    {
                        ...
                        LogWarning("Reach Cone {point " + i + ", point " + nextPoint +
        ...
        public void ResetToDefault()
        {
            ...
            for (var i = 0; i < points.Length; i++) points[i] = new LimitPoint();

            var swing1Rotation = Quaternion.AngleAxis(45, Vector3.right);
            var swing2Rotation = Quaternion.AngleAxis(45, Vector3.up);
        ...
        public void BuildReachCones()
        {
            ...
            for (var i = 0; i < points.Length; i++) P[i] = points[i].point.normalized;

            for (var i = 0; i < smoothIterations; i++) P = SmoothPoints();

            // Calculating the reach cones
            reachCones = new ReachCone[P.Length];
            for (var i = 0; i < reachCones.Length - 1; i++)
            ...
            for (var i = 0; i < reachCones.Length; i++) reachCones[i].Calculate();
        }
        ...
        private Vector3[] SmoothPoints()
        {
            ...
            var Q = new Vector3[P.Length * 2];

            var scalar = GetScalar(P.Length); // Get the constant used for interpolation

            // Project all the existing points on a plane that is tangent to the unit sphere at the Axis point
            for (var i = 0; i < Q.Length; i += 2) Q[i] = PointToTangentPlane(P[i / 2], 1);

            // Interpolate the new points
            for (var i = 1; i < Q.Length; i += 2)
            {
                var minus2 = Vector3.zero;
                var plus1 = Vector3.zero;
                var plus2 = Vector3.zero;
                ...
                var t = Q.Length / points.Length;

                // Interpolation
                Q[i] = 0.5f * (Q[i - 1] + plus1) + scalar * points[i / t].tangentWeight * (plus1 - minus2) +
                       scalar * points[i / t].tangentWeight * (Q[i - 1] - plus2);
            }

            // Project the points from tangent plane to the sphere
            for (var i = 0; i < Q.Length; i++) Q[i] = TangentPointToSphere(Q[i], 1);
        ...
        {
            var d = Vector3.Dot(q - axis, q - axis);
            var u = 4 * r * r / (4 * r * r + d);
            return u * q + (1 - u) * -axis;
        }
        ...
        private Quaternion LimitSwing( Quaternion rotation )
        {
            ...
            var L = rotation * axis; // Test this vector against the reach cones

            var r = GetReachCone(L); // Get the reach cone to test against (can be only 1)
            ...
            var toLimits = Quaternion.FromToRotation(rotation * axis, L);
        ...
        {
            float p = 0;
            var p1 = Vector3.Dot(reachCones[0].S, L);
