...
namespace RootMotion.FinalIK
{
    ...
    public class RotationLimitHinge : RotationLimit
    {
        ...
        private Quaternion LimitHinge( Quaternion rotation )
        {
            ...
            var free1DOF = Limit1DOF(rotation, axis);
            if (!useLimits) return free1DOF;

            // Get offset from last rotation in angle-axis representation
            var addR = free1DOF * Quaternion.Inverse(lastRotation);

            var addA = Quaternion.Angle(Quaternion.identity, addR);

            var secondaryAxis = new Vector3(axis.z, axis.x, axis.y);
            var cross = Vector3.Cross(secondaryAxis, axis);
