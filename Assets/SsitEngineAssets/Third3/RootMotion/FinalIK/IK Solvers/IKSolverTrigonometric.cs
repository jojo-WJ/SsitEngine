    ...
    public class IKSolverTrigonometric : IKSolver
    {
        ...
        protected override void OnUpdate()
        {
            ...
            if (IKPositionWeight > 0)
            {
                ...
                var currentBendNormal =
                    Vector3.Lerp(bone1.GetBendNormalFromCurrentRotation(), bendNormal, IKPositionWeight);

                // Calculating and interpolating bend direction
                var bendDirection = Vector3.Lerp(bone2.transform.position - bone1.transform.position,
                    GetBendDirection(weightIKPosition, currentBendNormal), IKPositionWeight);
        ...
        {
            var direction = IKPosition - bone1.transform.position;
            if (direction == Vector3.zero) return Vector3.zero;

            var directionSqrMag = direction.sqrMagnitude;
            var directionMagnitude = (float) Math.Sqrt(directionSqrMag);

            var x = (directionSqrMag + bone1.sqrMag - bone2.sqrMag) / 2f / directionMagnitude;
            var y = (float) Math.Sqrt(Mathf.Clamp(bone1.sqrMag - x * x, 0, Mathf.Infinity));

            var yDirection = Vector3.Cross(direction, bendNormal);
        ...
        public void SetBendGoalPosition( Vector3 goalPosition, float weight )
        {
            ...
            var normal = Vector3.Cross(goalPosition - bone1.transform.position, IKPosition - bone1.transform.position);
        ...
        public void SetBendPlaneToCurrent()
        {
            ...
            var normal = Vector3.Cross(bone2.transform.position - bone1.transform.position,
        ...
        public override bool IsValid( ref string message )
        {
            ...
            var duplicate = (Transform) Hierarchy.ContainsDuplicate(new Transform[3]
        ...
        public class TrigonometricBone : Bone
        {
            ...
            public void Initiate( Vector3 childPosition, Vector3 bendNormal )
            {
                ...
                var defaultTargetRotation = Quaternion.LookRotation(childPosition - transform.position, bendNormal);
        ...
        /// </summary>
        public static void Solve( Transform bone1, Transform bone2, Transform bone3, Vector3 targetPosition,
            Vector3 bendNormal, float weight )
        {
            if (weight <= 0f) return;
            ...
            var dir = targetPosition - bone1.position;

            // Distance between the first and the last node solver positions
            var length = dir.magnitude;
            if (length == 0f) return;

            var sqrMag1 = (bone2.position - bone1.position).sqrMagnitude;
            var sqrMag2 = (bone3.position - bone2.position).sqrMagnitude;
            ...
            var q1 = Quaternion.FromToRotation(bone2.position - bone1.position, toBendPoint);
            if (weight < 1f) q1 = Quaternion.Lerp(Quaternion.identity, q1, weight);

            bone1.rotation = q1 * bone1.rotation;

            var q2 = Quaternion.FromToRotation(bone3.position - bone2.position, targetPosition - bone2.position);
        ...
        private static Vector3 GetDirectionToBendPoint( Vector3 direction, float directionMag, Vector3 bendDirection,
            float sqrMag1, float sqrMag2 )
        {
