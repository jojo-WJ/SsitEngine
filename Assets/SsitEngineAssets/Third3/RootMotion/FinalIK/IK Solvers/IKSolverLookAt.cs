...
namespace RootMotion.FinalIK
{
    ...
    public class IKSolverLookAt : IKSolver
    {
        ...
        protected override void OnInitiate()
        {
            ...
            foreach (var s in spine) s.Initiate(root);
            if (head != null) head.Initiate(root);
            foreach (var eye in eyes) eye.Initiate(root);
        ...
        private void SolveSpine()
        {
            ...
            var targetForward = (IKPosition + spineTargetOffset - spine[spine.Length - 1].transform.position)
                .normalized;
            ...
            for (var i = 0; i < spine.Length; i++) spine[i].LookAt(spineForwards[i], bodyWeight * IKPositionWeight);
        }
        ...
        private void SolveHead()
        {
            ...
            var targetForward = Vector3.Lerp(baseForward, (IKPosition - head.transform.position).normalized,
        ...
        private void SolveEyes()
        {
            ...
            for (var i = 0; i < eyes.Length; i++)
            {
                ...
                var baseForward = head.transform != null ? head.forward : eyes[i].forward;
            ...
            float clamp )
        {
            ...
            {
                for (var i = 0; i < forwards.Length; i++) forwards[i] = baseForward;
                return forwards;
            }

            // Get normalized dot product. 
            var angle = Vector3.Angle(baseForward, targetForward);
            var dot = 1f - angle / 180f;

            // Clamping the targetForward so it doesn't exceed clamp
            var targetClampMlp = clamp > 0 ? Mathf.Clamp(1f - (clamp - dot) / (1f - dot), 0f, 1f) : 1f;

            // Calculating the clamp multiplier
            var clampMlp = clamp > 0 ? Mathf.Clamp(dot / clamp, 0f, 1f) : 1f;
            ...
            {
                var step = 1f / (forwards.Length - 1);

                // Calculate the forward for each bone
                for (var i = 0; i < forwards.Length; i++)
                    forwards[i] = Vector3.Slerp(baseForward, targetForward,
                        spineWeightCurve.Evaluate(step * i) * clampMlp * targetClampMlp);
            }
        ...
        public void SetLookAtWeight( float weight, float bodyWeight = 0f, float headWeight = 1f,
            float eyesWeight = 0.5f, float clampWeight = 0.5f, float clampWeightHead = 0.5f,
        ...
        {
            for (var i = 0; i < spine.Length; i++) spine[i].StoreDefaultLocalState();
            for (var i = 0; i < eyes.Length; i++) eyes[i].StoreDefaultLocalState();
        ...
        public override void FixTransforms()
        {
            ...
            for (var i = 0; i < spine.Length; i++) spine[i].FixTransform();
            for (var i = 0; i < eyes.Length; i++) eyes[i].FixTransform();
        ...
        public override bool IsValid( ref string message )
        {
            ...
            var spineDuplicate = ContainsDuplicateBone(spine);
            if (spineDuplicate != null)
            {
                message = spineDuplicate.name +
                          " is represented multiple times in a single IK chain. Can't initiate solver.";
                return false;
            }
            var eyeDuplicate = ContainsDuplicateBone(eyes);
            if (eyeDuplicate != null)
            {
                message = eyeDuplicate.name +
                          " is represented multiple times in a single IK chain. Can't initiate solver.";
                return false;
            }
            return true;
        ...
        {
            var allPoints = new Point[spine.Length + eyes.Length + (head.transform != null ? 1 : 0)];
            for (var i = 0; i < spine.Length; i++) allPoints[i] = spine[i];

            var eye = 0;
            for (var i = spine.Length; i < allPoints.Length; i++)
            {
        ...
        public class LookAtBone : Bone
        {
            ...
            {
                var fromTo = Quaternion.FromToRotation(forward, direction);
