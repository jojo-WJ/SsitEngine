    ...
    public abstract class IKSolver
    {
        ...
        public class Bone : Point
        {
            ...
            public void Swing( Vector3 swingTarget, float weight = 1f )
            {
                ...
                var r = Quaternion.FromToRotation(transform.rotation * axis, swingTarget - transform.position);
            ...
            public void Swing2D( Vector3 swingTarget, float weight = 1f )
            {
                ...
                var from = transform.rotation * axis;
                var to = swingTarget - transform.position;

                var angleFrom = Mathf.Atan2(from.x, from.y) * Mathf.Rad2Deg;
                var angleTo = Mathf.Atan2(to.x, to.y) * Mathf.Rad2Deg;

                transform.rotation = Quaternion.AngleAxis(Mathf.DeltaAngle(angleFrom, angleTo) * weight, Vector3.back) *
                                     transform.rotation;
            }
        ...
        public static bool HierarchyIsValid( Bone[] bones )
        {
            for (var i = 1;
                i < bones.Length;
                i++) // If parent bone is not an ancestor of bone, the hierarchy is invalid
