...
namespace RootMotion.FinalIK
{
    ...
    public partial class IKSolverVR : IKSolver
    {
        ...
        private void Write()
        {
            ...
            foreach (var leg in legs) leg.Write(ref solvedPositions, ref solvedRotations);
            foreach (var arm in arms) arm.Write(ref solvedPositions, ref solvedRotations);
        }
        ...
        private Vector3 GetPelvisOffset()
        {
            ...
            var sampledOrigin = raycastOriginPelvis;
            sampledOrigin.y = spine.pelvis.solverPosition.y;
            var origin = spine.pelvis.readPosition;
            origin.y = spine.pelvis.solverPosition.y;
            var direction = origin - sampledOrigin;
            RaycastHit hit;
            ...
            {
                if (Physics.Raycast(sampledOrigin, direction, out hit, direction.magnitude * 1.1f,
                    locomotion.blockingLayers)) origin = hit.point;
            }
            else
            {
                if (Physics.SphereCast(sampledOrigin, locomotion.raycastRadius * 1.1f, direction, out hit,
                    direction.magnitude, locomotion.blockingLayers))
                    origin = sampledOrigin + direction.normalized * hit.distance / 1.1f;
            }

            var position = spine.pelvis.solverPosition;
            ...
            {
                if (Physics.SphereCast(origin, locomotion.raycastRadius, direction, out hit, direction.magnitude,
                    locomotion.blockingLayers)) position = origin + direction.normalized * hit.distance;
            }
        ...
        public void DefaultAnimationCurves()
        {
            ...
            if (locomotion.stepHeight.keys.Length == 0) locomotion.stepHeight.keys = GetSineKeyframes(0.03f);

            if (locomotion.heelHeight.keys.Length == 0) locomotion.heelHeight.keys = GetSineKeyframes(0.03f);
        }
        ...
        {
            for (var i = 1; i < solverTransforms.Length; i++)
        ...
        public override void FixTransforms()
        {
            ...
            for (var i = 1; i < solverTransforms.Length; i++)
                if (solverTransforms[i] != null)
                {
                    var isPelvis = i == 1;
                    var isArm = i > 5 && i < 14;
                    if (isPelvis || isArm) solverTransforms[i].localPosition = defaultLocalPositions[i - 1];
        ...
        private readonly Quaternion[] defaultLocalRotations = new Quaternion[21];
        private readonly Vector3[] defaultLocalPositions = new Vector3[21];
        ...
        private Vector3 GetNormal( Transform[] transforms )
        {
            ...
            for (var i = 0; i < transforms.Length; i++) centroid += transforms[i].position;
            centroid /= transforms.Length;

            for (var i = 0; i < transforms.Length - 1; i++)
                normal += Vector3.Cross(transforms[i].position - centroid, transforms[i + 1].position - centroid)
                    .normalized;
        ...
        {
            var toForearm = forearm.position - hand.position;
            var axis = AxisTools.ToVector3(AxisTools.GetAxisToDirection(hand, toForearm));
        ...
        private Vector3 GuessPalmToThumbAxis( Transform hand, Transform forearm )
        {
            ...
            var closestSqrMag = Mathf.Infinity;
            var thumbIndex = 0;

            for (var i = 0; i < hand.childCount; i++)
            {
                var sqrMag = Vector3.SqrMagnitude(hand.GetChild(i).position - hand.position);
            ...
            var handNormal = Vector3.Cross(hand.position - forearm.position,
                hand.GetChild(thumbIndex).position - hand.position);
            var toThumb = Vector3.Cross(handNormal, hand.position - forearm.position);
            var axis = AxisTools.ToVector3(AxisTools.GetAxisToDirection(hand, toThumb));
        ...
        {
            for (var i = 0; i < solverTransforms.Length; i++)
                if (solverTransforms[i] != null)
                {
        ...
        private void WriteTransforms()
        {
            for (var i = 0; i < solverTransforms.Length; i++)
                if (solverTransforms[i] != null)
                {
                    var isRootOrPelvis = i < 2;
                    var isArm = i > 5 && i < 14;
                    if (isRootOrPelvis || isArm)
                        solverTransforms[i].position = V3Tools.Lerp(solverTransforms[i].position, GetPosition(i),
                            IKPositionWeight);

                    solverTransforms[i].rotation =
                        QuaTools.Lerp(solverTransforms[i].rotation, GetRotation(i), IKPositionWeight);
                }
        }
        ...
        private void Read( Vector3[] positions, Quaternion[] rotations, bool hasChest, bool hasNeck, bool hasShoulders,
            bool hasToes )
        {
            if (rootBone == null)
        ...
        private void Solve()
        {
            // Pre-Solving
            spine.PreSolve();
            foreach (var arm in arms) arm.PreSolve();
            foreach (var leg in legs) leg.PreSolve();

            // Applying spine and arm offsets
            foreach (var arm in arms) arm.ApplyOffsets();
            spine.ApplyOffsets();
            ...
            if (locomotion.weight > 0f)
            {
                ...
                var leftFootOffset = 0f;
                var rightFootOffset = 0f;
                var leftHeelOffset = 0f;
                var rightHeelOffset = 0f;
                ...
                rightLeg.footPositionOffset += (rightFootPosition - rightLeg.lastBone.solverPosition) *
                                               IKPositionWeight * (1f - rightLeg.positionWeight) * locomotion.weight;
                ...
                var rotationOffsetLeft = QuaTools.FromToRotation(leftLeg.lastBone.solverRotation, leftFootRotation);
                var rotationOffsetRight = QuaTools.FromToRotation(rightLeg.lastBone.solverRotation, rightFootRotation);

                rotationOffsetLeft = Quaternion.Lerp(Quaternion.identity, rotationOffsetLeft,
                ...
                var footPositionC = Vector3.Lerp(leftLeg.position + leftLeg.footPositionOffset,
                    rightLeg.position + rightLeg.footPositionOffset, 0.5f);
                footPositionC = V3Tools.PointToPlane(footPositionC, rootBone.solverPosition, root.up);

                var p = rootBone.solverPosition + rootVelocity * Time.deltaTime * 2f * locomotion.weight;
                ...
                var rootVelocityV = V3Tools.ExtractVertical(rootVelocity, root.up, 1f);
                rootVelocity -= rootVelocityV;

                var bodyYOffset = leftFootOffset + rightFootOffset;
            ...
            // Legs
            foreach (var leg in legs) leg.ApplyOffsets();

            if (!plantFeet)
            {
                spine.InverseTranslateToHead(legs, false, false, bodyOffset, 1f);

                foreach (var leg in legs) leg.TranslateRoot(spine.pelvis.solverPosition, spine.pelvis.solverRotation);
                foreach (var leg in legs) leg.Solve();
            }
            else
            {
                for (var i = 0; i < 2; i++)
            ...
            for (var i = 0; i < arms.Length; i++) arms[i].Solve(i == 0);

            // Reset offsets
            spine.ResetOffsets();
            foreach (var leg in legs) leg.ResetOffsets();
            foreach (var arm in arms) arm.ResetOffsets();
            ...
            supportLegIndex = -1;
            var shortestMag = Mathf.Infinity;
            for (var i = 0; i < legs.Length; i++)
            {
                var mag = Vector3.SqrMagnitude(legs[i].lastBone.solverPosition - legs[i].bones[0].solverPosition);
                if (mag < shortestMag)
                {
                    supportLegIndex = i;
                    shortestMag = mag;
