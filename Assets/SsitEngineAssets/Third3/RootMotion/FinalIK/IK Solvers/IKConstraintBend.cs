...
namespace RootMotion.FinalIK
{
    ...
    public class IKConstraintBend
    {
        ...
        public void Initiate( IKSolverFullBody solver )
        {
            ...
            var defaultNormal = Vector3.Cross((bone3.position - bone1.position).normalized, direction);
        ...
        public void LimitBend( float solverWeight, float positionWeight )
        {
            ...
            var normalDirection = bone1.rotation * -defaultLocalDirection;

            var axis2 = bone3.position - bone2.position;

            // Clamp the direction from knee/elbow to foot/hand to valid range (90 degrees from right-angledly bent limb)
            var changed = false;
            var clampedAxis2 = V3Tools.ClampDirection(axis2, normalDirection, clampF * solverWeight, 0, out changed);

            var bone3Rotation = bone3.rotation;

            if (changed)
            {
                var f = Quaternion.FromToRotation(axis2, clampedAxis2);
            ...
            if (positionWeight > 0f)
            {
                ...
                var q = Quaternion.FromToRotation(tangent, normalDirection);
        ...
        public Vector3 GetDir( IKSolverFullBody solver )
        {
            ...
            var w = weight * solver.IKPositionWeight;

            // Apply the bend goal
            if (bendGoal != null)
            {
                var b = bendGoal.position - solver.GetNode(chainIndex1, nodeIndex1).solverPosition;
                if (b != Vector3.zero) direction = b;
            }

            if (w >= 1f) return direction.normalized;

            var solverDirection = solver.GetNode(chainIndex3, nodeIndex3).solverPosition -
                                  solver.GetNode(chainIndex1, nodeIndex1).solverPosition;

            // Get rotation from animated limb direction to solver limb direction
            var f = Quaternion.FromToRotation(bone3.position - bone1.position, solverDirection);

            // Rotate the default bend direction by f
            var dir = f * (bone2.position - bone1.position);

            // Effector rotation
            if (solver.GetNode(chainIndex3, nodeIndex3).effectorRotationWeight > 0f)
            {
                // Bend direction according to the effector rotation
                var effectorDirection = -Vector3.Cross(solverDirection,
                    solver.GetNode(chainIndex3, nodeIndex3).solverRotation * defaultChildDirection);
                dir = Vector3.Lerp(dir, effectorDirection,
                    solver.GetNode(chainIndex3, nodeIndex3).effectorRotationWeight);
            }

            // Rotation Offset
            if (rotationOffset != Quaternion.identity)
            {
                var toOrtho = Quaternion.FromToRotation(rotationOffset * solverDirection, solverDirection);
