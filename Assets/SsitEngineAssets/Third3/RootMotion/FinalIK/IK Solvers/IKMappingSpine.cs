...
namespace RootMotion.FinalIK
{
    ...
    public class IKMappingSpine : IKMapping
    {
        ...
        {
            for (var i = 0; i < spine.Length; i++) spine[i].FixTransform(i == 0 || i == spine.Length - 1);
        }
        ...
        public override void Initiate( IKSolverFullBody solver )
        {
            ...
            for (var i = 0; i < spine.Length; i++) spine[i].SetIKPosition();
        ...
        {
            var firstPosition = spine[0].GetPlanePosition(solver);
            var rootPosition = solver.GetNode(spine[rootNodeIndex].chainIndex, spine[rootNodeIndex].nodeIndex)
                .solverPosition;
            var lastPosition = spine[spine.Length - 1].GetPlanePosition(solver);
            ...
            if (useFABRIK)
            {
                ...
                for (var i = 0; i < spine.Length; i++) spine[i].ikPosition = spine[i].transform.position + offset;
        ...
        public void ForwardReach( Vector3 position )
        {
            ...
            for (var i = spine.Length - 2; i > -1; i--) // Finding joint positions
        ...
        private void MapToSolverPositions( IKSolverFullBody solver )
        {
            ...
            for (var i = 1; i < spine.Length - 1; i++)
            {
                ...
                if (twistWeight > 0)
                {
                    var bWeight = i / ((float) spine.Length - 2);

                    var s1 = solver.GetNode(leftUpperArm.chainIndex, leftUpperArm.nodeIndex).solverPosition;
                    var s2 = solver.GetNode(rightUpperArm.chainIndex, rightUpperArm.nodeIndex).solverPosition;
                    spine[i].Twist(s1 - s2, spine[i + 1].ikPosition - spine[i].transform.position,
                        bWeight * twistWeight);
                }
            }
        ...
        /// </summary>
        [Range(1, 3)] public int iterations = 3;
        ...
        public override bool IsValid( IKSolver solver, ref string message )
        {
            ...
            foreach (var spineBone in spineBones)
                if (spineBone == null)
                {
