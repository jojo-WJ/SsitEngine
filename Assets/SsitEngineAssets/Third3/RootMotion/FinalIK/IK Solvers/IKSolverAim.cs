...
namespace RootMotion.FinalIK
{
    ...
    public class IKSolverAim : IKSolverHeuristic
    {
        ...
        protected override void OnUpdate()
        {
            ...
            var dir = clampedIKPosition - transform.position;
        ...
        private Vector3 GetClampedIKPosition()
        {
            ...
            var angle = Vector3.Angle(transformAxis, IKPosition - transform.position);
            ...
            return transform.position + Vector3.Slerp(transformAxis * 10f, IKPosition - transform.position,
                       clampMlp * targetClampMlp);
        }
        ...
        private void RotateToTarget( Vector3 targetPosition, Bone bone, float weight )
        {
            ...
            if (XY)
            {
                ...
                */
                ...
                if (weight >= 0f)
                {
                    var dir = transformAxis;
                    var targetDir = targetPosition - transform.position;

                    var angleDir = Mathf.Atan2(dir.x, dir.y) * Mathf.Rad2Deg;
                    var angleTarget = Mathf.Atan2(targetDir.x, targetDir.y) * Mathf.Rad2Deg;

                    bone.transform.rotation =
                        Quaternion.AngleAxis(Mathf.DeltaAngle(angleDir, angleTarget), Vector3.back) *
                        bone.transform.rotation;
                }
            }
            else
            {
                if (weight >= 0f)
                {
                    var rotationOffset = Quaternion.FromToRotation(transformAxis, targetPosition - transform.position);

                    if (weight >= 1f)
                        bone.transform.rotation = rotationOffset * bone.transform.rotation;
                    else
                        bone.transform.rotation = Quaternion.Lerp(Quaternion.identity, rotationOffset, weight) *
                                                  bone.transform.rotation;
                }
                ...
                if (poleWeight > 0f)
                {
                    var poleDirection = polePosition - transform.position;

                    // Ortho-normalize to transform axis to make this a twisting only operation
                    var poleDirOrtho = poleDirection;
                    var normal = transformAxis;
                    Vector3.OrthoNormalize(ref normal, ref poleDirOrtho);

                    var toPole = Quaternion.FromToRotation(transformPoleAxis, poleDirOrtho);
                    bone.transform.rotation = Quaternion.Lerp(Quaternion.identity, toPole, weight * poleWeight) *
                                              bone.transform.rotation;
                }
            }
