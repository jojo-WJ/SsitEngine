...
namespace RootMotion.FinalIK
{
    ...
    public partial class IKSolverVR : IKSolver
    {
        ...
        public class Leg : BodyPart
        {
            ...
            {
                var thighPos = positions[index];
                var thighRot = rotations[index];
                var calfPos = positions[index + 1];
                var calfRot = rotations[index + 1];
                var footPos = positions[index + 2];
                var footRot = rotations[index + 2];
                var toePos = positions[index + 3];
                var toeRot = rotations[index + 3];
            ...
            public override void PreSolve()
            {
                ...
                if (positionWeight > 0f) ApplyPositionOffset(IKPosition - position, positionWeight);
            ...
            public override void ApplyOffsets()
            {
                ...
                var fromTo = Quaternion.FromToRotation(footPosition - position,
                ...
                {
                    var b = Vector3.Cross(bendGoal.position - thigh.solverPosition, position - thigh.solverPosition);
                    var l = Quaternion.LookRotation(bendNormal, thigh.solverPosition - foot.solverPosition);
                    var bRelative = Quaternion.Inverse(l) * b;
                    bAngle = Mathf.Atan2(bRelative.x, bRelative.z) * Mathf.Rad2Deg * bendGoalWeight;
                }

                var sO = swivelOffset + bAngle;

                if (sO != 0f)
                {
                    bendNormal = Quaternion.AngleAxis(sO, thigh.solverPosition - lastBone.solverPosition) * bendNormal;
                    thigh.solverRotation = Quaternion.AngleAxis(-sO, thigh.solverRotation * thigh.axis) *
            ...
            {
                if (weight <= 0f) return;
                if (weight < 1f) offset = Quaternion.Lerp(Quaternion.identity, offset, weight);
            ...
            public void Solve()
            {
                ...
                var b = Vector3.Cross(foot.solverPosition - thigh.solverPosition,
                ...
                var calfRotation = thigh.solverRotation * calfRelToThigh;
                var fromTo =
                    Quaternion.FromToRotation(calfRotation * calf.axis, foot.solverPosition - calf.solverPosition);
