...
namespace RootMotion.FinalIK
{
    ...
    public class IKSolverCCD : IKSolverHeuristic
    {
        ...
        private void Solve( Vector3 targetPosition )
        {
            ...
            if (XY)
                for (var i = bones.Length - 2; i > -1; i--)
                {
                    ...
                    if (w > 0f)
                    {
                        var toLastBone = bones[bones.Length - 1].transform.position - bones[i].transform.position;
                        var toTarget = targetPosition - bones[i].transform.position;

                        var angleToLastBone = Mathf.Atan2(toLastBone.x, toLastBone.y) * Mathf.Rad2Deg;
                        var angleToTarget = Mathf.Atan2(toTarget.x, toTarget.y) * Mathf.Rad2Deg;
            ...
            // 3D
                ...
                for (var i = bones.Length - 2; i > -1; i--)
                {
                    ...
                    if (w > 0f)
                    {
                        ...
                        var targetRotation = Quaternion.FromToRotation(toLastBone, toTarget) *
                                             bones[i].transform.rotation;
        ...
        public void FadeOutBoneWeights()
        {
            ...
            bones[0].weight = 1f;
            var step = 1f / (bones.Length - 1);

            for (var i = 1; i < bones.Length; i++) bones[i].weight = step * (bones.Length - 1 - i);
        }
