...
namespace RootMotion.FinalIK
{
    ...
    public class IKSolverFullBodyBiped : IKSolverFullBody
    {
         ...
         * */
        private void SetLimbOrientation( FullBodyBipedChain chain,
            BipedLimbOrientations.LimbOrientation limbOrientation )
        {
            var inverse = chain == FullBodyBipedChain.LeftArm || chain == FullBodyBipedChain.RightArm;
        ...
        protected override void ReadPose()
        {
            ...
            for (var i = 0; i < effectors.Length; i++) effectors[i].SetToTarget();

            // Pulling the body with the hands
            PullBody();

            // Spine stiffness
            var s = Mathf.Clamp(1f - spineStiffness, 0f, 1f);
        ...
        private void PullBody()
        {
            ...
            if (pullBodyVertical != 0f || pullBodyHorizontal != 0f)
            {
                ...
                pullBodyOffset = V3Tools.ExtractVertical(offset, root.up, pullBodyVertical) +
                                 V3Tools.ExtractHorizontal(offset, root.up, pullBodyHorizontal);
        ...
        private Vector3 GetHandBodyPull( IKEffector effector, FBIKChain arm, Vector3 offset )
        {
            ...
            var direction = effector.position - (arm.nodes[0].transform.position + offset);
            var armLength = arm.nodes[0].length + arm.nodes[1].length;

            // Find delta of effector distance and arm length
            var dirMag = direction.magnitude;

            if (dirMag < armLength) return Vector3.zero;
            var x = dirMag - armLength;
        ...
        protected override void ApplyBendConstraints()
        {
            ...
            else
            {
                ...
                for (var i = 0; i < 5; i++) effectors[i].GetNode(this).solverPosition += offset;
            }
        ...
        {
            if (iterations == 0) spineMapping.spineBones[0].position += offset;
        ...
        public void SetToReferences( BipedReferences references, Transform rootNode = null )
        {
            ...
            if (effectors.Length != 9)
                ...
                {
                    new IKEffector(), new IKEffector(), new IKEffector(), new IKEffector(), new IKEffector(),
                    new IKEffector(), new IKEffector(), new IKEffector(), new IKEffector()
                };
            ...
            for (var i = 0; i < references.spine.Length; i++) spineBones[i + 1] = references.spine[i];
            ...
            // IKMappingBone
            var boneMappingsCount = references.head != null ? 1 : 0;

            if (boneMappings.Length != boneMappingsCount)
            {
                boneMappings = new IKMappingBone[boneMappingsCount];
                for (var i = 0; i < boneMappings.Length; i++) boneMappings[i] = new IKMappingBone();
        ...
        public static Transform DetectRootNodeBone( BipedReferences references )
        {
            ...
            var spineLength = references.spine.Length;
            if (spineLength == 1) return references.spine[0];

            var hip = Vector3.Lerp(references.leftThigh.position, references.rightThigh.position, 0.5f);
            var neck = Vector3.Lerp(references.leftUpperArm.position, references.rightUpperArm.position, 0.5f);
            var toNeck = neck - hip;
            var toNeckMag = toNeck.magnitude;

            if (references.spine.Length < 2) return references.spine[0];

            var rootNodeBone = 0;

            for (var i = 1; i < spineLength; i++)
            {
                var hipToBone = references.spine[i].position - hip;
                var projection = Vector3.Project(hipToBone, toNeck);

                var dot = Vector3.Dot(projection.normalized, toNeck.normalized);
                if (dot > 0)
                {
                    var mag = projection.magnitude / toNeckMag;
