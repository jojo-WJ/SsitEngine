    ...
    public class RagdollUtility : MonoBehaviour
    {
        ...
        private bool ikUsed
        {
            get
            {
                ...
                foreach (var k in allIKComponents)
        ...
        public void Start()
        {
            ...
            var firstIndex = rigidbodies[0].gameObject == gameObject ? 1 : 0;

            rigidbones = new Rigidbone[firstIndex == 0 ? rigidbodies.Length : rigidbodies.Length - 1];

            for (var i = 0; i < rigidbones.Length; i++) rigidbones[i] = new Rigidbone(rigidbodies[i + firstIndex]);

            // Find all the child Transforms
            var C = GetComponentsInChildren<Transform>();
            children = new Child[C.Length - 1];

            for (var i = 0; i < children.Length; i++) children[i] = new Child(C[i + 1]);
        }
        ...
        private IEnumerator DisableRagdollSmooth()
        {
            ...
            for (var i = 0; i < rigidbones.Length; i++) rigidbones[i].r.isKinematic = true;
        ...
        private void Update()
        {
            ...
            else
            {
                ...
                if (enableIK)
                {
                    ...
                    for (var i = 0; i < allIKComponents.Length; i++) disabledIKComponents[i] = false;
                }
            }
        }
        ...
        private void LateUpdate()
        {
            ...
            if (animator.updateMode != AnimatorUpdateMode.AnimatePhysics ||
                animator.updateMode == AnimatorUpdateMode.AnimatePhysics && fixedFrame) AfterAnimation();
        ...
        private void AfterAnimation()
        {
            if (isRagdoll
                ...
                StoreLocalState();
            else // Blending from ragdoll to animation. When ragdollWeight is zero, nothing happens here
        ...
        private void RagdollEnabler()
        {
            ...
            for (var i = 0; i < rigidbones.Length; i++) rigidbones[i].WakeUp(applyVelocity, applyAngularVelocity);

            // Remember some variables so we can revert to them when coming back from ragdoll
            for (var i = 0; i < fixTransforms.Length; i++)
            {
        ...
        {
            foreach (var r in rigidbones) r.RecordVelocity();
        }

        // Stored the current pose of the character
        private void StoreLocalState()
        {
            foreach (var c in children) c.StoreLocalState();
        }

        // Interpolate the character to the last stored pose (see StoreLocalState)
        private void FixTransforms( float weight )
        {
            foreach (var c in children) c.FixTransform(weight);
        }
        ...
        public class Rigidbone
        {
            ...
            public void WakeUp( float velocityWeight, float angularVelocityWeight )
            {
                ...
                if (updateAnchor) joint.connectedAnchor = t.InverseTransformPoint(c.position);

                r.isKinematic = false;

                // Transfer velocity from animation
                if (velocityWeight != 0f) r.velocity = deltaPosition / deltaTime * velocityWeight;
