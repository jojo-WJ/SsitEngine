...
namespace RootMotion
{
    ...
    public class AxisTools
    {
        ...
        {
            var axis = GetAxisVectorToPoint(t, worldPosition);
        ...
        {
            var axis = GetAxisVectorToDirection(t, direction);
            if (axis == Vector3.right) return Axis.X;
            if (axis == Vector3.up) return Axis.Y;
            return Axis.Z;
        }
        ...
        public static Vector3 GetAxisVectorToDirection( Transform t, Vector3 direction )
        {
            ...
            var dotX = Mathf.Abs(Vector3.Dot(Vector3.Normalize(t.right), direction));
            var dotY = Mathf.Abs(Vector3.Dot(Vector3.Normalize(t.up), direction));
            if (dotY > dotX) axis = Vector3.up;
            var dotZ = Mathf.Abs(Vector3.Dot(Vector3.Normalize(t.forward), direction));
