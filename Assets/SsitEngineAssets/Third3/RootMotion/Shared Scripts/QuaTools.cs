...
namespace RootMotion
{
    ...
    public static class QuaTools
    {
        ...
        {
            var fromTo = Quaternion.FromToRotation(fromDirection, toDirection);

            float angle = 0;
            var freeAxis = Vector3.zero;

            fromTo.ToAngleAxis(out angle, out freeAxis);

            var dot = Vector3.Dot(freeAxis, axis);
        ...
        {
            var closest = Vector3.right;
            var neg = false;

            var x = Vector3.Dot(v, Vector3.right);
            var maxAbsDot = Mathf.Abs(x);
            if (x < 0f) neg = true;

            var y = Vector3.Dot(v, Vector3.up);
            var absDot = Mathf.Abs(y);
            if (absDot > maxAbsDot)
            {
                maxAbsDot = absDot;
                closest = Vector3.up;
            ...
            var z = Vector3.Dot(v, Vector3.forward);
            absDot = Mathf.Abs(z);
            if (absDot > maxAbsDot)
            {
        ...
        public static Quaternion ClampRotation( Quaternion rotation, float clampWeight, int clampSmoothing )
        {
            ...
            var targetClampMlp = Mathf.Clamp(1f - (clampWeight - dot) / (1f - dot), 0f, 1f);
            var clampMlp = Mathf.Clamp(dot / clampWeight, 0f, 1f);
        ...
        public static float ClampAngle( float angle, float clampWeight, int clampSmoothing )
        {
            ...
            var dot = 1f - Mathf.Abs(angle) / 180f;
            var targetClampMlp = Mathf.Clamp(1f - (clampWeight - dot) / (1f - dot), 0f, 1f);
            var clampMlp = Mathf.Clamp(dot / clampWeight, 0f, 1f);
        ...
        /// </summary>
        public static Quaternion MatchRotation( Quaternion targetRotation, Vector3 targetforwardAxis,
            Vector3 targetUpAxis, Vector3 forwardAxis, Vector3 upAxis )
        {
            var f = Quaternion.LookRotation(forwardAxis, upAxis);
            var fTarget = Quaternion.LookRotation(targetforwardAxis, targetUpAxis);

            var d = targetRotation * fTarget;
