...
namespace RootMotion
{
    ...
    public class BipedReferences
    {
            ...
            AutoDetectParams autoDetectParams )
        {
            ...
            var isLegsParent = bone == references.leftThigh.parent;
        ...
        {
            var limb = BipedNaming.GetBonesOfTypeAndSide(boneType, boneSide, transforms);
        ...
        private static bool LimbError( Transform bone1, Transform bone2, Transform bone3, ref string errorMessage )
        {
            ...
            var duplicate = (Transform) Hierarchy.ContainsDuplicate(new Transform[3] {bone1, bone2, bone3});
            if (duplicate != null)
            {
        ...
        {
            var cross = Vector3.Cross(bone2.position - bone1.position, bone3.position - bone1.position);
        ...
        private static bool SpineError( BipedReferences references, ref string errorMessage )
        {
            ...
            for (var i = 0; i < references.spine.Length; i++)
            ...
            var duplicate = (Transform) Hierarchy.ContainsDuplicate(references.spine);
            if (duplicate != null)
            {
            ...
            for (var i = 0; i < references.spine.Length; i++)
            {
                var matchesParentPosition = false;
                if (i == 0 && references.spine[i].position == references.pelvis.position) matchesParentPosition = true;
                if (i != 0 && references.spine.Length > 1 &&
                    references.spine[i].position == references.spine[i - 1].position) matchesParentPosition = true;

                if (matchesParentPosition)
                {
                    errorMessage = "Biped's spine bone nr " + i +
        ...
        private static bool EyesError( BipedReferences references, ref string errorMessage )
        {
            ...
            for (var i = 0; i < references.eyes.Length; i++)
            ...
            var duplicate = (Transform) Hierarchy.ContainsDuplicate(references.eyes);
            if (duplicate != null)
            {
        ...
        private static bool RootHeightWarning( BipedReferences references, ref string warningMessage )
        {
            ...
            var headHeight = GetVerticalOffset(references.head.position, references.leftFoot.position,
                references.root.rotation);
            var rootHeight = GetVerticalOffset(references.root.position, references.leftFoot.position,
                references.root.rotation);
        ...
        {
            var handsLeftToRight = references.rightHand.position - references.leftHand.position;
            var feetLeftToRight = references.rightFoot.position - references.leftFoot.position;

            var dotHands = Vector3.Dot(handsLeftToRight.normalized, references.root.right);
            var dotFeet = Vector3.Dot(feetLeftToRight.normalized, references.root.right);
        ...
        {
            var v = Quaternion.Inverse(rotation) * (p1 - p2);
            return v.y;
        }
            ...
            AutoDetectParams autoDetectParams )
        {
            ...
            if (SetupWarning(references, ref message)) Warning.Log(message, references.root, true);
            ...
            AutoDetectParams autoDetectParams )
        {
            ...
            {
                var neck = Hierarchy.GetFirstCommonAncestor(references.leftUpperArm, references.rightUpperArm);

                if (neck != null)
                {
                    var inverseSpine = new Transform[1] {neck};
                    Hierarchy.AddAncestors(inverseSpine[0], references.pelvis, ref inverseSpine);

                    references.spine = new Transform[0];
                    for (var i = inverseSpine.Length - 1; i > -1; i--)
                    ...
                    // Head
                    if (references.head == null)
                        for (var i = 0; i < neck.childCount; i++)
                        {
                            var child = neck.GetChild(i);
            ...
            // Find eye bones
            var eyes = BipedNaming.GetBonesOfType(BipedNaming.BoneType.Eye, children);
        ...
        public static bool SetupError( BipedReferences references, ref string errorMessage )
        {
            ...
            if (LimbError(references.rightThigh, references.rightCalf, references.rightFoot, ref errorMessage))
                return true;
            if (LimbError(references.leftUpperArm, references.leftForearm, references.leftHand,
                ref errorMessage)) return true;
            if (LimbError(references.rightUpperArm, references.rightForearm, references.rightHand,
                ref errorMessage)) return true;
        ...
        public static bool SetupWarning( BipedReferences references, ref string warningMessage )
        {
            if (LimbWarning(references.leftThigh, references.leftCalf, references.leftFoot, ref warningMessage))
                return true;
            if (LimbWarning(references.rightThigh, references.rightCalf, references.rightFoot,
                ref warningMessage)) return true;
            if (LimbWarning(references.leftUpperArm, references.leftForearm, references.leftHand,
                ref warningMessage)) return true;
