...
namespace Framework.SceneObject
{
    ...
    {
        public Player( string mGuid ) : base(mGuid)
        {
        ...
        protected override void InitFsm()
        {
            mFsm = FsmHelper.CreateFsm(Guid
                ...
                , new CharacterStateAssign(this)
                , new CharacterStateStrecher(this)
                //, new CharacterStateStrecherWait(this)
                , new CharacterStateDetectorGas(this)
                , new CharacterStateMHQAttach(this)
                , new CharacterStateMHQReady(this)
                //, new CharacterStateMHQFire(this) //取消
                //, new CharacterStateWPipeAttach(this)
                , new CharacterStateWPipeReady(this)
                , new CharacterStateWPipeFire(this)
                , new CharacterStateSentry(this)
                , new CharacterStateXfp(this)
                , new CharacterStateInteractoin(this)
            );
            //access: 等待状态接入
        ...
        {
            if (mRepresent != null && m_playerController != null)
            {
                if (!mRepresent.activeSelf)
                    return;
                sceneInstance?.OnUpdate();
                m_playerController.OnUpdate(elapsed);
                UpdateFsm(elapsed);
            }
        }
        ...
        public override void Shutdown()
        {
            base.Shutdown();
            DestoryFsm();
            sceneInstance?.SetLink(false, null);
            sceneInstance = null;
            mUsingItems = null;
            mData = null;
        ...
        protected override void OnCreateComplete( GameObject represent )
        {
            if (represent != null)
            {
                ...
                // playerController
                var agent = mRepresent.GetComponent<NetPlayerAgent>();
                if (agent) agent.SetLink(true, this);
