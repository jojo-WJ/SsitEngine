...
namespace Framework.SceneObject
{
    ...
    public class PlayerInstance : BaseSceneInstance
    {
        ...
        public new Player LinkObject => m_baseObject as Player;
        ...
        public override void OnSelect( bool state, object select )
        {
            ...
            if (state && HasAuthority)
                GlobalManager.Instance.Player = LinkObject;
            else if (select != null)
                if (select is BaseSceneInstance sceneObject && sceneObject.CanEdit)
        ...
        public override void OnChangePropertyCallback( EnPropertyId propertyId, string property, object data = null )
        {
            switch (propertyId)
            {
                case EnPropertyId.Active:
                {
                    ...
                    if (!active && GlobalManager.Instance.Player == LinkObject)
                        GlobalManager.Instance.Player = null;
                }
                    break;
