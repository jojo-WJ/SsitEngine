...
*│　作   者：xuXin                                              
*│　版   本：1.0.0                                                 
*│　创建时间：2019/6/3 11:30:12                     
*└──────────────────────────────────────────────────────────────┘
*/
//#define EnableLookVector

using System.Collections.Generic;
using SsitEngine.Unity.Unity.Msic;
using UnityEngine;
...
namespace SsitEngine.Unity.SsitInput
{
    ...
    /// </summary>
    public abstract class SsitInput : MonoBase
    {
        #region 摄像机朝向机制检测
        ...
        public string HorizontalLookInputName { get { return m_HorizontalLookInputName; } set { m_HorizontalLookInputName
 = value; } }

        public string VerticalLookInputName { get { return m_VerticalLookInputName; } set { m_VerticalLookInputName =
 value; } }
        ...
        public float LookAccelerationThreshold { get { return m_LookAccelerationThreshold; } set { m_LookAccelerationThreshold
 = value; } }
        ...
        private void UpdateLookVector()
        {
            ...
            if (m_LookVectorMode == LookVectorMode.Smoothed)
            {
                ...
                var averageTotal = 0f;
                var deltaTime = m_TimeScale * GameTime.FramerateDeltaTime;
        ...
        [SerializeField]
        protected float m_ControllerConnectedCheckRate = 1f;

        [Tooltip("The state that should be activated when a controller is connected.")] [SerializeField]
        protected string m_ConnectedControllerState = "ConnectedController";

        [Tooltip("Unity event invoked when the gameplay input is enabled or disabled.")] [SerializeField]
        protected UnityBoolEvent m_EnableGamplayInputEvent;
        ...
        public float ControllerConnectedCheckRate
        {
            get => m_ControllerConnectedCheckRate;
            set => m_ControllerConnectedCheckRate = value;
        }

        public string ConnectedControllerState
        {
            get => m_ConnectedControllerState;
            set => m_ConnectedControllerState = value;
        }
        ...
        {
            return GetButtonDownInternal(name);
        }
        ...
        {
            return GetButtonUpInternal(name);
        }
        ...
        public bool GetDoublePress( string name )
        {
            ...
            {
                if (m_ButtonDownTime == null) m_ButtonDownTime = new Dictionary<string, float>();
                var time = -1f;
                if (m_ButtonDownTime.TryGetValue(name, out time))
                {
                    if (time != Time.unscaledTime && time + 0.2f > Time.unscaledTime) return true;
        ...
        public bool GetTap( string name )
        {
            ...
            {
                if (m_ButtonDownTime == null) m_ButtonDownTime = new Dictionary<string, float>();
                if (!m_ButtonDownTime.ContainsKey(name)) m_ButtonDownTime.Add(name, Time.unscaledTime);
            }
        ...
        public bool GetLongPress( string name, float duration, bool waitForRelease )
        {
            ...
            if (GetButtonInternal(name))
            {
                ...
                if (m_ButtonDownTime.TryGetValue(name, out downTime))
                {
                    ...
                    if (upTime > downTime) m_ButtonDownTime[name] = downTime = Time.unscaledTime;
                    // Return true as soon as the button has been pressed for the duration.
                    if (!waitForRelease) return downTime + duration <= Time.unscaledTime;
                }
                else
                {
                    m_ButtonDownTime.Add(name, Time.unscaledTime);
        ...
        {
            return GetAxisRawInternal(name);
        }
        ...
        /// <param name="name">The name of the button.</param>
        /// <returns>True of the button is being pressed.</returns>
        protected virtual bool GetButtonInternal( string name )
        {
            return false;
        }
        ...
        /// <param name="name">The name of the button.</param>
        /// <returns>True if the button is pressed this frame.</returns>
        protected virtual bool GetButtonDownInternal( string name )
        {
            return false;
        }
        ...
        /// <param name="name">The name of the button.</param>
        /// <returns>True if the button is up.</returns>
        protected virtual bool GetButtonUpInternal( string name )
        {
            return false;
        }
        ...
        /// <param name="name">The button name to check for a double press.</param>
        /// <returns>True if a double press occurred (double click or double tap).</returns>
        protected virtual bool GetDoublePressInternal( string name )
        {
            return false;
        }
        ...
        /// <param name="name">The name of the axis.</param>
        /// <returns>The value of the axis.</returns>
        protected virtual float GetAxisInternal( string name )
        {
            return 0;
        }
        ...
        /// <param name="name">The name of the axis.</param>
        /// <returns>The value of the raw axis.</returns>
        protected virtual float GetAxisRawInternal( string name )
        {
            return 0;
        }
