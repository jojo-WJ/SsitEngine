...
{
    private static readonly float textLableWidth = 150;
    private static readonly float enumWidth = 300;
    private static readonly float elementHeight = 40;
    ...
    {
        InitData();
        mTexture = AssetDatabase.LoadAssetAtPath<Texture2D>("Assets/SsitEngineAssets/Editor/Res/Line.png");
            ...
            ( rect, index, isActive, isFocused ) =>
            {
                ...
                var rectControl = rect;
                {
                    ...
                    if (element.go != null)
                    {
                        ...
                        var compStrings = components.ToList().ConvertAll(x => { return x.GetType().Name; }).Where(
                            component =>
                            {
                                if (!string.IsNullOrEmpty(element.filterStr) && !Regex.IsMatch(component,
                                        element.filterStr, RegexOptions.IgnoreCase))
                                {
                                    return false;
                                }
                                return true;
                        ...
                        {
                            element.typeName = compStrings[element.selectIndex];
                        }
                    }
                    rectControl.x += enumWidth;

                    element.filterStr = EditorGUI.TextField(
                        new Rect(rectControl.x, rectControl.y, textLableWidth * 2, EditorGUIUtility.singleLineHeight),
                        new GUIContent("快速定位"), element.filterStr);

                    EditorGUI.DrawTextureTransparent(
                        new Rect(rect.x, rect.y + EditorGUIUtility.singleLineHeight, rect.width, 0.5f)
                    ...
                    element.fieldName = EditorGUI.TextField(
                        new Rect(rectControl.x, rectControl.y, textLableWidth * 2, EditorGUIUtility.singleLineHeight),
                    ...
                    EditorGUI.LabelField(
                        new Rect(rectControl.x, rectControl.y, textLableWidth, EditorGUIUtility.singleLineHeight),
    ...
    private void DrawToolbar()
    {
        ...
        */
        ...
        mGo = EditorGUILayout.ObjectField("UI Form: ", mGo, typeof(GameObject), true, GUILayout.MinWidth(200)) as
            GameObject;
        if (EditorGUI.EndChangeCheck())
        {
            InitBindForm();
    ...
    private void DrawPreview()
    {
        ...
        EditorGUILayout.LabelField(new GUIContent(m_stringBuilder.ToString()), new GUIStyle("en_text"),
    ...
    private void DrawCheck()
    {
        ...
        //检测绑定对象名称唯一性
        var bindNames = mSeriableData.Where(item => item.go).ToList().ConvertAll(x => { return x.go.name; });
    ...
    private void ParseCodeRes( string scripPath )
    {
        ...
        if (beginIndex != -1)
        {
            ...
            for (var i = 0; i < splitStr.Length; i++)
            {
                ...
                {
                    var tt = splitStr[i].Split(new[] {"\"", "\""}, StringSplitOptions.RemoveEmptyEntries);
                    ...
                    {
                        tt = splitStr[i + 1].Split(new[] {" ", ";"}, StringSplitOptions.RemoveEmptyEntries);
                        item.typeName = tt[1];
                        item.fieldName = tt[2];
                    }
    ...
    public class SeriableItem
    {
        ...
        [NonSerialized] public string filterStr;

        [NonSerialized] public GameObject go;

        [NonSerialized] public bool isVaild;
    ...
    private void ExportCode()
    {
        ...
        else
        {
            ...
            if (!string.IsNullOrEmpty(mScripPath))
            {
                var assetPath = mScripPath.Replace(Application.dataPath, ""); //.Replace(".cs","");
    ...
    private string FindScript( string scriptName )
    {
        ...
        if (Directory.Exists(tempStr))
        {
            ...
            {
                if (files[i].Name.EndsWith(".cs") && Path.GetFileNameWithoutExtension(files[i].Name) == scriptName)
                {
